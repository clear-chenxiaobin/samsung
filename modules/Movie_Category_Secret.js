...
angular.module('app.movie', [])
    ...
    .controller('MovieController', ['$scope', '$http','ResourceManager', 'ActivityManager', 'COMMON_KEYS','MovieService','MenuService', function ($scope, $http,ResourceManager, ActivityManager, COMMON_KEYS,MovieService,MenuService) {
        ...
        getData();
        function getData() {
            MovieService.getMovieData().success(function (data) {
                $scope.movieData = data;
                if($scope.movieData){
                    loadData();
                }
            });
            //$scope.movieData = childData;
            //loadData();
        }
        function loadData(){
        ...
        function choseMovie(typeID,movieID) {
            ...
            $scope.movieName = $scope.movie[typeID][movieID].name;
            $scope.movieDirector = $scope.movie[typeID][movieID].director;
            $scope.movieActor = $scope.movie[typeID][movieID].actor;
            $scope.movieIntro = $scope.movie[typeID][movieID].intro;
...
.service('MovieService', ['$q', '$http', 'ResourceManager', function ($q, $http, ResourceManager) {
    ...
    this.getMovieData = function () {
        return $http.get('http://192.168.30.75/nativevod/now/Video/json/Movie_Category_Secret_167.json').success(function(data){
            mData = data;
        });
    };
